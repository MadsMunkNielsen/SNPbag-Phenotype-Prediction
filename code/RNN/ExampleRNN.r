...
# Add reverse edges between middle nodes
reverse_edges <- data.frame(
  from = c("h2", "h3", "y3"),
  to   = c("h1", "h2", "h3"),
  label = c("W[hh]", "W[hh]", "yback")
)

# Combine with original edges
edges_all <- bind_rows(edges, reverse_edges)
...
# Edge going into h1 (above, now h[t-2])
coords_in <- shorten(x_left_long, y_left + 0.12, nodes$x[nodes$id == "h1"], nodes$y[nodes$id == "h1"] + 0.12, r)
edge_in <- tibble(
...
# Edge going out of h1 (below, now W[hh])
coords_out <- shorten(nodes$x[nodes$id == "h1"], nodes$y[nodes$id == "h1"] - 0.12, x_left_long, y_left - 0.12, r)
edge_out <- tibble(
  x = coords_out[1], y = coords_out[2],
  xend = coords_out[3], yend = coords_out[4],
  label = "W[hh]"
)

# Combine and set label offsets: h[t-2] above, W[hh] below
double_edges_split <- bind_rows(edge_in, edge_out)
double_edges_split$label_offset <- c(0.18, -0.18)

# Add a label_offset column for vertical adjustment
double_edges_split$label_offset <- c(0.18, -0.18) # Adjust these values as needed
...
# Separate W[hh] edges for custom label placement
edges_coords_W_hh <- edges_coords_all[edges_coords_all$label == "W[hh]", ]

# Separate yback edge for custom label placement
edges_coords_yback <- edges_coords_all[edges_coords_all$label == "yback", ]

# Plot
Plot_RNN_Extra <- ggplot() +
  geom_circle(data=nodes, aes(x0=x, y0=y, r=r), fill="white", color="black") +
  ...
  geom_text(data=extra_edge,
            ...
            parse=TRUE, size=6, vjust=3.1, hjust=0.6) +
  geom_text(data=edges_coords_W[edges_coords_W$label != "W[hh]", ],
            aes(x=(x+xend)/2, y=(y+yend)/2, label=label),
            parse=TRUE, size=6, vjust=0.6, hjust=1.3) +
  geom_text(data=edges_coords_W_hh,
            aes(x=(x+xend)/2, y=(y+yend)/2 + 0.2, label=label),
            parse=TRUE, size=6, vjust=3.1, hjust=0.5) +
  geom_text(data=edges_coords_h,
            aes(x=(x+xend)/2, y=(y+yend)/2, label=label),
            parse=TRUE, size=6, vjust=-0.5, hjust=0.5) +
  geom_text(data=edges_coords_yback,
            aes(x=(x+xend)/2 + 0.15, y=(y+yend)/2, label=label),
            parse=TRUE, size=6, vjust=0.5, hjust=0) +
...
pdf("fig/img/RNN/RNN_Example_Extra2.pdf", width=10, height=6)
Plot_RNN_Extra2
dev.off()