...
\end{align*}
Since $\tanh: \R \to (-1, 1)$, the resulting hidden state for the $n$'th observation $h_n \in (-1, 1)^p$. From each of the hidden states, sequential estimates can be made
\begin{align*}
    \hat{y}_n = g_\Theta(h_n),
\end{align*}
where $g_\Theta :\R^p \to \R^q$ is an invariant activation function parameterized by $\Theta = \{V, b_y\}$, where $V \in \R^{q \times p}$ is a weight matrix between the hidden state and the output, and $b_y \in \R^q$ is a bias term. If the outputs should be represented as a probability distribution, the activation function, $g_\Theta (\cdot)$, is often chosen as the softmax function. Thus, the estimates for each of the $n$ inputs can be described as
\begin{align*}
    \hat{y}_{n,j} = \text{softmax}_\Theta\left(h_n \right)= \frac{\exp((Vh_{n} + b_y)_j)}{\sum^q_{k=1}\exp((Vh_{n} + b_y)_k)},
\end{align*}
where $\hat{y}_{n,j}$ denotes the j'th estimate for the n'th input in the sequence. 
\begin{figure}
    \centering
